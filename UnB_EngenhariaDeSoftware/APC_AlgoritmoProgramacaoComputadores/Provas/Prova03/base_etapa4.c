...
encontra_voo_mais_cheio(tPedido *pass, int num){
    int mais_pass, y, array_mais;
    int codigo_mais;

    mais_pass = pass[0].passageiros;
    array_mais = 0;

    for(y = 1; y <= num; y++){
        if(pass[y].passageiros > mais_pass){
            mais_pass = pass[y].passageiros;
            array_mais = y;
        }
    }
    //codigo_mais = pass[array_mais].num_voo;
    return array_mais;
